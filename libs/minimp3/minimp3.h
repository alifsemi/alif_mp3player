...
#if !defined(MINIMP3_ONLY_SIMD) && (defined(_M_X64) || defined(__x86_64__) || defined(__aarch64__) || defined(_M_ARM64) || (__ARM_FEATURE_MVE & 2))
/* x64 always have SSE2, arm64 always have neon, MVE is compile-time enabled, no need for generic code */
...
#define VMAC_S(a, x, s) _mm_add_ps(a, _mm_mul_ps(x, _mm_set1_ps(s)))
#define VMSB_S(a, x, s) _mm_sub_ps(a, _mm_mul_ps(x, _mm_set1_ps(s)))
#define VREV(x) _mm_shuffle_ps(x, x, _MM_SHUFFLE(0, 1, 2, 3))
#define VSTORE_REV(m, x) VSTORE(m, VREV(x))
#define VLD_REV(m) VREV(VLD(m))
typedef __m128 f4;
...
#define VMAC_S(a, x, s)  vmlaq_f32(a, x, vmovq_n_f32(s))
#define VMSB_S(a, x, s)  vmlsq_f32(a, x, vmovq_n_f32(s))
...
#elif __ARM_FEATURE_MVE & 2
#include <arm_mve.h>
#define HAVE_SSE 0
#define HAVE_MVE 1
#define HAVE_SIMD 1
#define VSTORE vstrwq_f32
#define VLD vldrwq_f32
#define VSET vdupq_n_f32
#define VADD vaddq_f32
#define VSUB vsubq_f32
#define VMUL vmulq_f32
#define VMAC(a, x, y) vfmaq_f32(a, x, y)
#define VMSB(a, x, y) vfmsq_f32(a, x, y)
#define VMUL_S(x, s)  vmulq_n_f32(x, s)
#define VMAC_S(a, x, s)  vfmaq_n_f32(a, x, s)
#define VMSB_S(a, x, s)  vfmaq_n_f32(a, x, -(s))
#define VREV(x) vcreateq_f32(vgetq_lane_u64(vreinterpretq_u64(vrev64q_f32(x)), 1), \
                             vgetq_lane_u64(vreinterpretq_u64(vrev64q_f32(x)), 0))
#if !defined __clang__
/* GCC seems to mess up either builtin_shuffle or scatter operations. Only the vcreateq form works */
#define VSTORE_REV(m, x) VSTORE(m, VREV(x))
#define VLD_REV(m) VREV(VLD(m))
#else
#define VSTORE_REV(m, x) vstrwq_scatter_shifted_offset_f32(m, vddupq_u32(3, 1), x)
#define VLD_REV(m) vldrwq_gather_shifted_offset_f32(m, vddupq_u32(3, 1))
#endif
typedef float32x4_t f4;
static int have_simd()
{
...
static void L3_antialias(float *grbuf, int nbands)
{
    ...
    for (; nbands > 0; nbands--, grbuf += 18)
    {
        ...
        if (have_simd()) for (; i < 8; i += 4)
        {
            ...
            f4 vd = VLD_REV(grbuf + 14 - i);
            f4 vc0 = VLD(g_aa[0] + i);
            f4 vc1 = VLD(g_aa[1] + i);
            VSTORE(grbuf + 18 + i, VMSB(VMUL(vu, vc0), vd, vc1));
            VSTORE_REV(grbuf + 14 - i, VMAC(VMUL(vu, vc1), vd, vc0));
        }
#endif /* HAVE_SIMD */
...
static void L3_imdct36(float *grbuf, float *overlap, const float *window, int nbands)
{
    ...
    for (j = 0; j < nbands; j++, grbuf += 18, overlap += 9)
    {
        ...
        if (have_simd()) for (; i < 8; i += 4)
        {
            ...
            f4 vsum = VMAC(VMUL(vc, vr1), vs, vr0);
            VSTORE(overlap + i, VMSB(VMUL(vc, vr0), vs, vr1));
            VSTORE(grbuf + i, VMSB(VMUL(vovl, vw0), vsum, vw1));
            VSTORE_REV(grbuf + 14 - i, VMAC(VMUL(vovl, vw1), vsum, vw0));
        }
#endif /* HAVE_SIMD */
        for (; i < 9; i++)
        {
...
#if HAVE_MVE
        mve_pred16_t pred = vctp32q(n - k);
        for (i = 0; i < 7; i++, y += 4*18)
        {
            f4 s = vaddq_x_f32(t[3][i], t[3][i + 1], pred);
            vst1q_p_f32(&y[0*18], t[0][i], pred);
            vst1q_p_f32(&y[1*18], vaddq_x_f32(t[2][i], s, pred), pred);
            vst1q_p_f32(&y[2*18], vaddq_x_f32(t[1][i], t[1][i + 1], pred), pred);
            vst1q_p_f32(&y[3*18], vaddq_x_f32(t[2][1 + i], s, pred), pred);
        }
        vst1q_p_f32(&y[0*18], t[0][7], pred);
        vst1q_p_f32(&y[1*18], vaddq_x_f32(t[2][7], t[3][7], pred), pred);
        vst1q_p_f32(&y[2*18], t[1][7], pred);
        vst1q_p_f32(&y[3*18], t[3][7], pred);
#else /* HAVE_MVE*/
        if (k > n - 3)
        {
#if HAVE_SSE
...
static int16_t mp3d_scale_pcm(float sample)
{
    ...
    /* MVE means you must also have this FPv5 instruction - don't know a more specific test */
    __asm__ ("vcvta.s32.f32 %0, %1" : "=t"(s32) : "t"(sample));
#else
    sample += .5f;
    s32 = ((int32_t) sample) - (sample <= 0);   /* away from zero, to be compliant */
#endif
    ...
    sample += .5f;
    int16_t s = ((int16_t) sample) - (sample <= 0);   /* away from zero, to be compliant */
#endif
    return s;
}
#else /* MINIMP3_FLOAT_OUTPUT */
...
static void mp3d_synth(float *xl, mp3d_sample_t *dstl, int nch, float *lins)
{
...
#if HAVE_MVE
    /* If nch == 1, dstr = dstl, and dstl stores overwrite dstr stores */
    /* If nch == 2, dstr = dstl + 1 */
#ifndef MINIMP3_FLOAT_OUTPUT
    const uint8_t c_offsets_stereo[8] = { 15*2, 17*2, 15*2+1, 17*2+1, 47*2, 49*2, 47*2+1,49*2+1 };
    uint16x8_t offsets = vldrbq_u16(c_offsets_stereo);
    /* Sneaky load of (-1, +1, -1, +1, -1, +1, -1, +1) */
    uint16x8_t pm = vreinterpretq_u16(vdupq_n_u32(0x0001FFFF));
    /* Squeeze offsets for mono - we'll predicate away the dstr stores */
    if (nch == 1)
        offsets = vshrq_n_u16(offsets, 1);
    else
        pm = vshlq_n_u16(pm, 1);
#else
    const uint8_t c_offsets_stereo[4] = { 15*2, 15*2+1, 47*2, 47*2+1 };
    uint32x4_t offsets = vldrbq_u32(c_offsets_stereo);
    if (nch == 1)
        offsets = vshrq_n_u32(offsets, 1);
#endif /* MINIMP3_FLOAT_OUTPUT */
#endif /* HAVE_MVE */
    ...
    {
#define VLOAD(k) float w0 = *w++; float w1 = *w++; f4 vz = VLD(&zlin[4*i - 64*k]); f4 vy = VLD(&zlin[4*i - 64*(15 - k)]);
#define V0(k) { VLOAD(k) b = VMAC_S(VMUL_S(   vz, w1), vy, w0); a = VMSB_S(VMUL_S(   vz, w0), vy, w1);  }
#define V1(k) { VLOAD(k) b = VMAC_S(VMAC_S(b, vz, w1), vy, w0); a = VMSB_S(VMAC_S(a, vz, w0), vy, w1); }
#define V2(k) { VLOAD(k) b = VMAC_S(VMAC_S(b, vz, w1), vy, w0); a = VMSB_S(VMAC_S(a, vy, w1), vz, w0); }
        f4 a, b;
        zlin[4*i]     = xl[18*(31 - i)];
...

        {
...
#elif HAVE_MVE
            uint16x8_t pos = vmlaq_n_u16(offsets, pm, i);
            int16x8_t pcm8 = vuninitializedq_s16();
            pcm8 = vqmovnbq_s32(pcm8, vcvtaq_s32_f32(a));
            pcm8 = vqmovntq_s32(pcm8, vcvtaq_s32_f32(b));
            vstrhq_scatter_shifted_offset_p_s16(dstl, pos, pcm8, nch == 1 ? 0x0F0F : 0xFFFF);
#else /* HAVE_SSE / MVE */
            int16x4_t pcma, pcmb;
            a = VADD(a, VSET(0.5f));
            b = VADD(b, VSET(0.5f));
            pcma = vqmovn_s32(vqaddq_s32(vcvtq_s32_f32(a), vreinterpretq_s32_u32(vcleq_f32(a, VSET(0)))));
            pcmb = vqmovn_s32(vqaddq_s32(vcvtq_s32_f32(b), vreinterpretq_s32_u32(vcleq_f32(b, VSET(0)))));
            ...
            a = VMUL_S(a, 1.0f/32768.0f);
            b = VMUL_S(b, 1.0f/32768.0f);
#if HAVE_SSE
...
#elif HAVE_MVE
            vstrwq_scatter_shifted_offset_p_f32(dstl -      i  * nch, offsets, a, nch == 1 ? 0x0F0F : 0xFFFF);
            vstrwq_scatter_shifted_offset_p_f32(dstl + (2 + i) * nch, offsets, b, nch == 1 ? 0x0F0F : 0xFFFF);
#else /* HAVE_SSE */
...
void mp3dec_f32_to_s16(const float *in, int16_t *out, int num_samples)
{
    ...
    {
        f4 a = VMUL_S(VLD(&in[i  ]), 32768.0f);
        f4 b = VMUL_S(VLD(&in[i+4]), 32768.0f);
#if HAVE_SSE
        static const f4 g_max = { 32767.0f, 32767.0f, 32767.0f, 32767.0f };
        static const f4 g_min = { -32768.0f, -32768.0f, -32768.0f, -32768.0f };
        __m128i pcm8 = _mm_packs_epi32(_mm_cvtps_epi32(_mm_max_ps(_mm_min_ps(a, g_max), g_min)),
...
#elif HAVE_MVE
        int16x8_t pcma = vqmovnbq_s32(vuninitializedq_s16(), vcvtaq_s32_f32(a));
        int16x8_t pcmb = vqmovnbq_s32(vuninitializedq_s16(), vcvtaq_s32_f32(b));
        vstrhq_s32(out+i,   vreinterpretq_s32(pcma));
        vstrhq_s32(out+i+4, vreinterpretq_s32(pcmb));
#else /* HAVE_SSE */
        int16x4_t pcma, pcmb;
        a = VADD(a, VSET(0.5f));
        b = VADD(b, VSET(0.5f));
    ...
    for(; i < num_samples; i++)
    {
        ...
        {
            sample += .5f;
            out[i] = ((int16_t) sample) - (sample <= 0); /* away from zero, to be compliant */
        }
    }
}
#endif /* MINIMP3_FLOAT_OUTPUT */
